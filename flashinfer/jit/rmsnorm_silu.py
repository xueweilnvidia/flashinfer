...
# Range-based knob LUT for SM90 (H100), tuned on WAN VAE image-derived shapes.
...
# "unbounded upper" sentinel. Generated by sweep_fused_rmsnorm_silu_knobs.py.
_KNOB_RANGE_LUT_SM90 = {
    # C=48  (anchors=[901120, 2088960])
    (48, 0, 1372007, "bf16"): (32, 0, 0, 2, 4),  # anchor=901120
    (48, 1372007, 2**31, "bf16"): (32, 0, 0, 2, 4),  # anchor=2088960
    # C=96  (anchors=[225280, 460800, 522240, 1296000])
    (96, 0, 322194, "bf16"): (4, 0, 1, 16, 2),  # anchor=225280
    (96, 322194, 490559, "bf16"): (8, 0, 1, 8, 2),  # anchor=460800
    (96, 490559, 822692, "bf16"): (8, 0, 1, 8, 2),  # anchor=522240
    (96, 822692, 2**31, "bf16"): (32, 0, 1, 6, 2),  # anchor=1296000
    # C=192  (anchors=[1760, 3520, 8160, 12240, 28160, 65280, 115200, 326400])
    (192, 0, 2489, "bf16"): (32, 0, 0, 1, 2),  # anchor=1760
    (192, 2489, 5359, "bf16"): (32, 0, 2, 3, 2),  # anchor=3520
    (192, 5359, 9993, "bf16"): (32, 0, 0, 3, 4),  # anchor=8160
    (192, 9993, 18565, "bf16"): (8, 0, 1, 8, 4),  # anchor=12240
    (192, 18565, 42875, "bf16"): (8, 0, 1, 16, 4),  # anchor=28160
    (192, 42875, 86719, "bf16"): (8, 0, 1, 16, 4),  # anchor=65280
    (192, 86719, 193910, "bf16"): (8, 0, 2, 10, 4),  # anchor=115200
    (192, 193910, 2**31, "bf16"): (8, 0, 2, 16, 4),  # anchor=326400
    # C=384  (anchors=[1760, 8160, 14080, 48960])
    (384, 0, 3789, "bf16"): (32, 0, 1, 8, 4),  # anchor=1760
    (384, 3789, 10718, "bf16"): (32, 0, 2, 2, 4),  # anchor=8160
    (384, 10718, 26255, "bf16"): (8, 0, 1, 16, 4),  # anchor=14080
    (384, 26255, 2**31, "bf16"): (32, 0, 1, 8, 8),  # anchor=48960
}
...
def _compute_default_knobs(C: int, dtype: str):
    ...
    The kernel emits a predicated partial tail LDG when ``VEC_COLS`` does not
    divide evenly by ``VEC_COLS_PER_LDG``, so the only remaining requirement
    for the bf16/fp8 paths is ``C % NUM_ELTS == 0`` (vectorized loads stay
    aligned).

    nvfp4 still requires ``VEC_COLS % VEC_COLS_PER_LDG == 0`` because the
    BlockScale store path in ``ln_fwd_silu_kernel.cuh`` performs warp-level
    cooperation that would diverge on a partial tail. nvfp4 also requires
    ``C % 32 == 0`` per the kernel's existing ``static_assert``.
    """
    input_size = 2  # bf16
